...
function bindEvents(){
    ...
    document.addEventListener("click",function(e){
        ...
        if(e.target.classList.contains("cp-i")){var pw=e.target.getAttribute("data-cp");if(navigator.clipboard){navigator.clipboard.writeText(pw).then(function(){toast("Copied!",true)})}else{var ta=document.createElement("textarea");ta.value=pw;ta.style.cssText="position:fixed;opacity:0";document.body.appendChild(ta);ta.select();document.execCommand("copy");document.body.removeChild(ta);toast("Copied!",true)}}
    });
}
...
function loadLeaks(){
    if(!db){$("#grid").innerHTML='<div class="empty"><i class="fas fa-exclamation-triangle"></i><h3>Error</h3><p>Could not connect to the database</p></div>';return}
    ...
    db.collection("lz_leaks").orderBy("t","desc").get().then(function(snap){
        ...
        render();stats();
    }).catch(function(e){console.error(e);$("#grid").innerHTML='<div class="empty"><i class="fas fa-exclamation-triangle"></i><h3>Loading Error</h3><p>Make sure Firestore is enabled and rules are open</p></div>'})
}
...
/* ===== VCC Generator ===== */
var VCC_NAMES=["John Smith","Jane Johnson","Michael Williams","Sarah Brown","David Jones","Emily Garcia","Robert Miller","Lisa Davis","James Rodriguez","Maria Martinez"];
var VCC_TYPES={visa:{pfx:["4"],len:16,name:"VISA",css:"visa"},mastercard:{pfx:["51","52","53","54","55"],len:16,name:"MASTERCARD",css:"mastercard"},amex:{pfx:["34","37"],len:15,name:"AMEX",css:"amex"},discover:{pfx:["6011"],len:16,name:"DISCOVER",css:"discover"}};

function vccGenNum(type){var ct=VCC_TYPES[type];if(!ct)return"";var pfx=ct.pfx[Math.floor(Math.random()*ct.pfx.length)];var num=pfx;while(num.length<ct.len-1)num+=Math.floor(Math.random()*10);var s=0,al=true;for(var i=num.length-1;i>=0;i--){var n=parseInt(num[i],10);if(al){n*=2;if(n>9)n-=9}s+=n;al=!al}return num+((10-(s%10))%10)}
...
function vccCard(type){var n=vccGenNum(type),ct=VCC_TYPES[type];return{num:n,fmt:vccFmt(n),exp:vccExp(),cvv:vccCvv(type),name:VCC_NAMES[Math.floor(Math.random()*VCC_NAMES.length)],type:type,brand:ct.name,css:ct.css}}
...
function buildVccPage(){
    ...
    h+=buildModals();app.innerHTML=h;bindEvents();bindVcc();vccSingle();
}

function bindVcc(){var g=document.getElementById("vccGen");if(g)g.addEventListener("click",vccSingle);var bg=document.getElementById("vccBG");if(bg)bg.addEventListener("click",vccBulk)}
...
function vccBulk(){
    ...
    var lines=[];lines.push("Brand     | Number                | Expiry  | CVV  | Name");lines.push("".padEnd(72,"-"));
